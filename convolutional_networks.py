...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    # Extract input and weight shape
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    out[n, f, i, j] = torch.sum(x_pad[n, :, h_start:h_end, w_start:w_end] * w[f]) + b[f]
        ...
        for f in range(F):
            for j in range(0, H + 2*pad - HH + 1, stride):
                for k in range(0, W + 2*pad - WW + 1, stride):
                    dw[f] += dout[i,f,j//stride,k//stride] * x_pad[i,:,j:j+HH,k:k+WW]
                    db[f] += dout[i,f,j//stride,k//stride]
                    dxp[i,:,j:j+HH,k:k+WW] += dout[i,f,j//stride,k//stride] * w[f]
...
class MaxPool(object):
  ...
  def backward(dout, cache):
    ...
    # 每個pooling area的max對應的梯度加到input的對應位置
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    window = x[n, c, h_start:h_end, w_start:w_end] # pooling區域的子矩陣
                    max_index = torch.argmax(window) # 找到window對應的index
                    max_index = (max_index // pool_width, max_index % pool_width) # //整數除法

                    dx[n, c, h_start:h_end, w_start:w_end][max_index] += dout[n, c, i, j] # 將梯度加到dx正確位置上
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = torch.randn(num_filters * H // 2 * W // 2, hidden_dim) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    # 前向傳播
    out, pool_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    out, linear_cache = Linear_ReLU.forward(out, W2, b2)
    ...
    daffine, grads['W3'], grads['b3'] = Linear.backward(dscore, cache)
    dpool, grads['W2'], grads['b2'] = Linear_ReLU.backward(daffine, linear_cache)

    dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dpool, pool_cache)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
      ...
      scores, cache = FastConv.forward(scores,self.params['W%d' % (i + 1)], 
                                   self.params['b%d' % (i + 1)], 
                                   conv_param)
      # print(i,"-th after conv out's shape:",scores.shape)
      ...
      if self.batchnorm:
        scores, cache = SpatialBatchNorm.forward(scores,self.params['gamma%d' % (i + 1)],
                                          ...
                                          self.bn_params[i])
        # print(i,"-th after batchnorm out's shape:",scores.shape)
        cache_history.append(cache)
      scores, cache = ReLU.forward(scores)
      cache_history.append(cache)
      # print(i,"-th after ReLU out's shape:",scores.shape)
      if i in self.max_pools:
        scores, cache = FastMaxPool.forward(scores,pool_param)
        cache_history.append(cache)
        # print(i,"-th after max_pools out's shape:",scores.shape)
    i+=1
    scores, cache = Linear.forward(scores,self.params['W%d' % (i + 1)], self.params['b%d' % (i + 1)] )
    # print(" after Final Linear out's shape:",scores.shape)
    cache_history.append(cache)
    L2reg += torch.sum(self.params['W%d' % (i + 1)] ** 2)
    L2reg *= self.reg
...
def find_overfit_parameters():
  ...
  # 透過weight scale，可以設置權重的初始化範圍，使權重能夠適當地初始化
  # weight scale 越小，權重的範圍就越小，有助於避免梯度爆炸；越大，有助於避免梯度消失
  weight_scale = 2e-1
  learning_rate = 1e-3
...
class BatchNorm(object):
  ...
  def backward_alt(dout, cache):
    ...
    dvar = (dxhat * (x-sample_mean) * (-0.5) * torch.pow(sample_var+eps, -1.5)).sum(axis = 0)  # (D,)
    dmean = torch.sum(dxhat * (-1) * torch.pow(sample_var + eps, -0.5), axis = 0)
    dmean += dvar * torch.sum(-2 * (x - sample_mean), axis = 0) / m
    dx = dxhat * torch.pow(sample_var + eps, -0.5) + dvar*2*(x - sample_mean) / m + dmean / m
    dgamma = torch.sum(dout * x_hat, axis = 0)
    dbeta = torch.sum(dout, axis = 0)
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    x = x.transpose(1,3).transpose(1,2).reshape(N * H * W, C)
